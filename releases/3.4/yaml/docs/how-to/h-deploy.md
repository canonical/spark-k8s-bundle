...
Charmed Spark comes with a bundled set of components that let you to easily 
...
that form Charmed Spark, please refer to the Explanation section.

## Prerequisites

Since Charmed Spark will be managed by Juju, make sure that:

* Juju client (e.g. via a [SNAP](https://snapcraft.io/juju)) is installed on your local machine.
* You can connect to a Juju controller.
...
to have a dedicated model for `Spark` components, for example:
...
Terraform modules make use of the Terraform Juju provider. More information about the Juju provider can be found in [Terraform registry](https://registry.terraform.io/providers/juju/juju/latest/docs).
...
To deploy Charmed Spark using Terraform, use standard TF syntax:
