...
2. Applications, outlining the product features that enable a secure deployment of a Apache Spark cluster
...
We recommend to deploy Charmed Apache Spark K8s with encryption enabled for securing the communication between components, whenever available and supported by the server and client applications.
