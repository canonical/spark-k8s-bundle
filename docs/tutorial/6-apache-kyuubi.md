...
```shell
juju deploy kyuubi-k8s --channel 4.0/edge --trust --config expose-external=loadbalancer
...
```{note}
The `kyuubi-k8s` charm in track `4.0` supports Apache Spark 4.0. If you'd like to use Apache Spark 3.5 or 3.4, use the charm
from tracks `3.5` or `3.4` respectively.
```
...
kyuubi-k8s       1.11     active      1  kyuubi-k8s       4.0/stable     181  10.152.183.84   no
