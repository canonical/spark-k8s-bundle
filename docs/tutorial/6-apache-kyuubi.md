...
```{note}
The `kyuubi-k8s` charm in track `3.4` supports Apache Spark 3.4. If you'd like to use Apache Spark 4.0 or 3.5, use the charm
from tracks `4.0` or `3.5` respectively.
```
...
auth-db          14.23    active      1  postgresql-k8s   14/stable      925  10.152.183.19   no
