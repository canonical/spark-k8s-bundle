...
class TestKyuubi:
    ...
    def test_authentication_is_enforced(self, juju: jubilant.Juju) -> None:
    ...
    def test_jdbc_endpoint(self, juju: jubilant.Juju) -> None:
        ...
        self_signed_certificate_unit = next(
            iter(status.apps["certificates"].units.keys())
        )
    ...
    def test_postgresql_metastore_is_used(
        ...
        with port_forward(
            pod=f"{METASTORE}-0", port=5432, namespace=cast(str, juju.model)
        ):
    ...
    def test_ha_deployment(self, juju: jubilant.Juju) -> None:
    ...
    def test_kyuubi_metrics_in_cos(self, juju: jubilant.Juju, cos) -> None:
        if not cos:
    ...
    def test_drop_table_if_exists(self, juju: jubilant.Juju) -> None:
        credentials = get_kyuubi_credentials(juju, "kyuubi")
        client = KyuubiClient(**credentials, use_ssl=True)
