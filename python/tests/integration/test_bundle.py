...
async def test_deploy_bundle(
        ops_test: OpsTest,
        credentials, bucket, registry, service_account, bundle
):
    """Deploy the bundle."""
    ...
    cmds = [
        "juju", "deploy", "--trust",
        "-m", ops_test.model_full_name, f"{bundle.relative_to(Path.cwd())}",
    ...
    await ops_test.model.wait_for_idle(
        apps=["s3"],
        timeout=2000, idle_period=30, status="blocked"
    )
    ...
    with ops_test.model_context("cos") as cos_model:
        await cos_model.wait_for_idle(
            apps=[
                "loki", "grafana", "prometheus", "catalogue",
    ...
    await ops_test.model.wait_for_idle(
        apps=applications,
        timeout=3600, idle_period=30, status="active"
    )
