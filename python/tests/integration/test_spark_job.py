...
class TestSparkJob:
    ...
    def test_run_job(
        ...
        driver_pod = [
            pod for pod in driver_pods if pod.pod_name not in initial_driver_pods
        ]
        ...
        line_check = filter(
            lambda line: "Number of lines" in line, driver_pod[0].logs()
        )
    ...
    def test_job_in_history_server(
        self, juju: jubilant.Juju, tmp_folder, port_forward: PortForwarder
    ) -> None:
        driver_pod = Pod.load(tmp_folder / "spark-job-driver.json")
    ...
    def test_job_not_in_prometheus_pushgateway(
        self, juju: jubilant.Juju, cos, tmp_folder: Path, port_forward: PortForwarder
    ) -> None:
    ...
    def test_spark_metrics_in_prometheus(
        ...
        with port_forward(
            pod=f"{PROMETHEUS}-0", port=9090, namespace=cos, on_port=19090
        ):
    ...
    def test_spark_logforwaring_to_loki(
        self, juju: jubilant.Juju, cos, port_forward: PortForwarder
    ) -> None:
        if not cos:
            pytest.skip("Not possible to test without cos")
